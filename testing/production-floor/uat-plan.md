# UAT Plan — Production Floor (`production_log`, module `Production Floor`)
...
Packaging changed on the day it was written — this shipped briefly as a
separate `vcl_production` app and is now a module inside `production_log` —
but **nothing in the scenarios below changed with it.** The routes, the
screen, the DocTypes and the report are identical. If you are holding a
printout that says "app", it is still valid.
