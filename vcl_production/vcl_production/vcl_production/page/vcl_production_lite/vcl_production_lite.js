...
class VclProductionBoard {
	...
	refresh() {
		return frappe
			.call({
				method: "vcl_production.api.get_board",
	...
	render_report() {
		...
		frappe
			.call({
				method: "vcl_production.api.get_report",
				args: { production_date: this.date },
			})
			.then((response) => {
	...
	render_history() {
		...
		frappe
			.call({ method: "vcl_production.api.get_history", args: { limit: 60 } })
			.then((response) => {
	...
	day_notes_dialog() {
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: (values) => {
				frappe
					.call({
						method: "vcl_production.api.set_day_notes",
	...
	remember_job(row, dialog) {
		frappe
			.call({
				method: "vcl_production.api.remember_current_job",
				args: {
	...
	close_day() {
		...
		frappe.confirm(message, () => {
			frappe
				.call({
					method: "vcl_production.api.close_day",
					args: { production_date: this.date },
					freeze: true,
				})
				.then((response) => {
	...
	reopen_day() {
		frappe.confirm(__("Reopen this production day for editing?"), () => {
			frappe
				.call({
					method: "vcl_production.api.reopen_day",
					args: { production_date: this.date },
					freeze: true,
				})
				.then((response) => {
	...
	render_recent_jobs(dialog) {
		...
		frappe
			.call({
				method: "vcl_production.api.suggest_jobs",
	...
	submit_add_job(dialog, values) {
		...
		frappe
			.call({
				method: "vcl_production.api.add_item",
				args: {
	...
	quick_update_dialog(row_name) {
		...
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: (values) => {
				...
				frappe
					.call({
						method: "vcl_production.api.update_item",
						args: {
							production_date: this.date,
							row: row.name,
			...
			secondary_action: () => {
				frappe.confirm(__("Remove this job from the day?"), () => {
					frappe
						.call({
							method: "vcl_production.api.remove_item",
